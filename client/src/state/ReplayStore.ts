...
import { draftStore, type DraftStore } from "./DraftStore";

import type { SelectedView } from "./selection";
...
import type { CardStore } from "@/draft/CardStore";
 ...
 */
export const replayStore = vuexModule(
  rootStore,
  "replay",
  {
    selectedDraft: 0,
    ...
    mutations: {
      ...
      goNext(state: ReplayState) {
        switch (state.timeMode) {
          ...
          case "synchronized":
            const seatId = replayStore.selection?.type == "seat" ? replayStore.selection.id : null;
            const targetEvent = getNextPickEvent(replayStore, seatId);

            commitWhile(state, (event) => targetEvent == null || !occursAfter(event, targetEvent));
      ...
      goBack(state: ReplayState) {
        switch (state.timeMode) {
          ...
          case "synchronized":
            ...
            rollbackWhile(
              state,
              (event) => targetEvent == null || !occursBefore(event, targetEvent),
            );
            break;
        ...
        if (mode == "synchronized") {
          ...
          for (; i < state.events.length; i++) {
            ...
            if (
              ...
              (event.round == currentRound && event.roundEpoch > currentEpoch)
            ) {
              break;
            }
            commitTimelineEvent(draftStore, event, state.draft);
...
function sortEventsLockstep(a: TimelineEvent, b: TimelineEvent) {
  ...
  if (a.associatedSeat != b.associatedSeat) {
    if (a.associatedSeat == -1) {
      return 1;
    } else if (b.associatedSeat == -1) {
      return 1;
    } else {
      return a.associatedSeat - b.associatedSeat;
    }
  }
...
function getCurrentEpoch(state: ReplayState): [number, number] {
  let currentRound = 1;
  let currentEpoch = 0;
  const mostRecentEvent = state.events[state.eventPos - 1] as TimelineEvent | undefined;
...
function freshJumpTo(draftStore: DraftStore, state: ReplayState, index: number) {
...
function consumeTransitionEvents(cardStore: CardStore, state: ReplayState) {
  for (let i = state.eventPos; i < state.events.length; i++) {
    const event = state.events[i];
