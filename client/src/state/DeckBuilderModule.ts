import type { DraftState, DraftCard } from "@/draft/DraftState";
 ...
 */
export const deckBuilderStore = vuexModule(
  rootStore,
  MODULE_NAME,
  {
    selectedSeat: 0,
  ...
  } as DeckBuilderState,
  {
    mutations: {
      sync(state: DeckBuilderState, draftState: DraftState) {
        ...
        for (const seat of draftState.seats) {
          init.push({
            draftName: draftStore.draftName,
            pool: seat.picks.cards.map((cardId) => draftStore.getCard(cardId)),
          });
        }
        state.decks = [];
        for (const [index, initializer] of init.entries()) {
          const draftName = initializer.draftName;
          const deck = getStoredDeck(draftName, index);
          ...
          for (const card of initializer.pool) {
            if (!cardsInDeck.has(card.id)) {
              deck.maindeck[Math.min(card.definition.cmc, deck.sideboard.length - 1)].push(card);
            }
          }
      ...
      moveCard(state: DeckBuilderState, move: CardMove) {
        ...
        const cards: DraftCard[] = move.source.map(
          (location) => sourceSection[location.columnIndex][location.cardIndex],
        );
        if (
          move.source.some(
            (location) =>
          ...
          move.target.cardIndex -= move.source.filter(
            (location) =>
              location.maindeck === move.target.maindeck &&
              location.columnIndex === move.target.columnIndex &&
        ...
        move.source.forEach((location, index) => {
          sourceSection[location.columnIndex].splice(
            sourceSection[location.columnIndex].indexOf(cards[index]),
            1,
          );
        });
        targetSection[move.target.columnIndex].splice(move.target.cardIndex, 0, ...cards);
        state.selection = [];
      },
      ...
      sortByCmc(state: DeckBuilderState, payload: { seat: number; maindeck: boolean }) {
        sort(state, payload.seat, payload.maindeck, (cards, numColumns) => {
          const newSection: CardColumn[] = (<DraftCard[][]>Array(numColumns))
            .fill([])
            .map(() => []);
      ...
      sortByColor(state: DeckBuilderState, payload: { seat: number; maindeck: boolean }) {
        sort(state, payload.seat, payload.maindeck, (cards, numColumns) => {
          ...
          for (const card of cards) {
            const isDfc = card.definition.card_faces.length > 0;
            const colors = isDfc ? card.definition.card_faces[0].colors : card.definition.colors;
            const type = isDfc
              ? card.definition.card_faces[0].type_line
              : card.definition.type_line;
            if (colors.length === 1) {
...
deckBuilderStore.subscribe((_mutation, state) => {
 ...
 */
function fillArray<T>(length: number, entryGenerator: (index: number) => T): T[] {
  const arr = [] as T[];
