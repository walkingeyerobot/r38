import { TimelineEvent, TimelineAction } from "../TimelineEvent";

export function eventToString(event: TimelineEvent) {
  return `[TimelineEvent id=${event.id}\n`
...
function actionToString(action: TimelineAction) {
  switch (action.type) {
    case 'move-card':
      return `${action.type} ${action.subtype} ${action.card} `
          + `"${action.cardName}" from=${action.from} to=${action.to}`;
    case 'move-pack':
      return `${action.type} pack=${action.pack} from=${action.from} `
          + `to=${action.to}`;
    default:
      return action.type;
  }
